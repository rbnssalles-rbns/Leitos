...
if uploaded_file is not None:
    ...
    def converter_tempo(x):
        try:
            if isinstance(x, pd.Timedelta):
                ...
                return float(x) * 24 * 3600  # Excel pode armazenar como fração de dia
        except:
            return None
        return None

    # Forçar leitura como string para garantir parsing
    ...
    # --- Conversão das novas colunas para segundos ---
    df["AGUARDANDO_SEGUNDOS"] = df["AGUARDANDO_LIMPEZA_51"].apply(converter_tempo)
    df["LIMPEZA_SEGUNDOS"] = df["EM_LIMPEZA_53"].apply(converter_tempo)
    df["CAMAREIRA_SEGUNDOS"] = df["CAMAREIRA_54"].apply(converter_tempo)
    ...
    # --- Chamadas para cada etapa do fluxo ---
    analisar_coluna(df_filtrado, "LIMPEZA_SEGUNDOS", "TEMPO_DE_LIMPEZA")
    analisar_coluna(df_filtrado, "AGUARDANDO_SEGUNDOS", "AGUARDANDO_LIMPEZA")
    analisar_coluna(df_filtrado, "CAMAREIRA_SEGUNDOS", "CAMAREIRA")
